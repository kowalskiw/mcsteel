...
import fires
from fdsafir2 import run_safir


'''New and more object-oriented code'''


def temperature_failure(theta_crit, time_temp_array):
    pass


class Scenario:
    def __init__(self, structure, fuel, beam_n_col=True):
        self.chid = ''
        self.fire_curve = ''
        self.path = ''
        self.fire = self.fire_parameters(fuel)
        self.sections = self.the_most_exposed(structure)    # pandasdf(?) array of sections to be calculated within this scenario
        # [{'chid':'dont know yet', 'path':'path\to\config\file', 'distance': XY fire-section distance

        self.bc_methodology = beam_n_col    # if you analyse the most exposed beam and column of each scenario
        self.results = []

    # create input files for the given case
    def prepare_files(self, section_data):
        # create locafi.txt
        # create dummy structural input basing on section_data
        # copy thermal input of given section from config to self.path
        pass

    # define fire parameters
    def fire_parameters(self, fuel):
        fire = {'xyz': (0, 0, 0)}
        # find fire parameters with fires.py

        return fire

    # find coordinates of the most exposed sections
    def the_most_exposed(self, geometry):
        sections = {}
        fc = self.fire['xyz']
        if self.bc_methodology:
            # repeat for beam and column
            #   finding algorithm
            #   if section is closer to the fire: replace
            pass
        else:
    ...
    # compare beam and column scenario
    def compare(self, to_be_compared):
        the_worst = len(to_be_compared)
        if the_worst == 1:
            return to_be_compared[0]
        else:
            for criterion in zip(*to_be_compared):
                max_of_crit = max(*criterion)
                if criterion.count(max_of_crit) == 1:
                    the_worst = criterion.index(max_of_crit)
                else:
                    continue

            return to_be_compared[the_worst]

    # sum up scenario results

    def get_results(self, chid, theta_crit):
        # find results in self.path\chid.tem
        with open('{}\\b00001_2.tem'.format(self.path, chid)) as file:
            tem = file.readlines()
        ...
        t = -1          # time
        temp = -500     # temperature sum
        nfiber = -1     # number of fibers in the section
            ...
            # add temperature in element in certain step
                ...
                try:
                    ...
                    if fiber_temp >= amb_temp:
                        temp += fiber_temp
                    else:   # check for temperature lower than ambient - it is known SAFIR error
                        print(
        ...
        time_list, temp_list = zip(*section_temp)

        res = [max(*temp_list), temperature_failure()]
        return res

    def run(self, safir_exe_path, all_sections=False):
        # run scenario
        s_res = []
        for case in self.sections if all_sections else [self.sections[0]]:
            c_res = []
            for el in case:     # beam and column or just the nearest section
                self.prepare_files(case)
                run_safir('{}\{}.in'.format(self.path, el['chid']), safir_exe_path=safir_exe_path, print_time=False)
                c_res.append(self.get_results(el['chid']))    # append element results to case results

            s_res.append(self.compare(c_res))



        return self.sum_result(self.compare() if self.bc_methodology else 0)



# read multisimulation config
def read_config(config_file_path):
    config_dict = {}
    with open(config_file_path) as file:
        config = file.readlines()

    # add elements to the config dict
...
class Multisimulation:
    def __init__(self, path_to_config):
        self.config = read_config(path_to_config)   # load multisimulation parameters and distributions of random values
        self.scenarios = []
        self.geometry = 'load dxf_geometry'
        self.fuel = 'load fuel location and properties'
        self.rmse = 1e6
    ...
    # check if rmse is lower than limit value
    def check_rmse(self, limit=1e-3):
        pass

    # generate the set of scenarios to be calculated
    def generate_scenarios(self, n):
        new_scenarios = []
        for i in range(n):
            new_scenarios.append(Scenario(self.geometry, self.fuel))

        self.scenarios.extend(new_scenarios)

        return  new_scenarios

    # default run of multisimulation
    def run(self, base_n=100):
        rmse_limit = False

        while not rmse_limit:
            # generate basic sample of scenarios
            self.generate_scenarios(base_n)

            # calclate generated scenarios
            temp_results = []
            for s in self.scenarios:
                temp_results.append(s.run())
                self.save_results(temp_results)
