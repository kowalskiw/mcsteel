import gmsh
import numpy as np


class Geometry:
    def __init__(self):
        gmsh.initialize()
        self.mesh_size = 0.1    # element size [m]
        self.nodes = list()
        self.lines = list()
        self.shells = list()
        self.volumes = list()

        self.sections = list()

    def get_node_id(self, idx: int): return self.nodes[idx]

    def get_node_tag(self, tag: int): return self.nodes[tag-1]

    def get_line_id(self, idx: int): return self.lines[idx]

    def get_line_tag(self, tag: int): return self.lines[abs(tag)-1]

    # def get_line_nodes(self, tag: int): return [self.nodes[node_tag] for node_tag in self.lines[tag]]

    # def get_line_startpoint(self, tag: int): return self.nodes[self.lines[tag][0]]

    # def get_line_endpoint(self, tag: int): return self.nodes[self.lines[tag][0]]

    def get_shell_id(self, idx: int): return self.shells[idx]

    def get_shell_tag(self, tag: int): return self.shells[tag-1]

    # maybe we don't need to mesh at all
    def _mesh(self):
        # exclude lines that are used for surfaces from meshing
        for surface in gmsh.model.getEntities(2):
            loop_tag, loop_lines = gmsh.model.occ.getCurveLoops(1)
            print(loop_lines)
            print(surface)
        # mesh lines only
        gmsh.model.mesh.setSize(gmsh.model.getEntities(0), self.mesh_size)
        gmsh.model.mesh.generate(1)

    def _import_nodes(self):
        entities = gmsh.model.getEntities(0)
        if not len(entities):
            raise Exception('[ERROR] No points found in geometry file')
        _, node_tags = zip(*entities)
        self.nodes = [Node(*gmsh.model.getValue(0,tag, [0]), tag=tag) for tag in node_tags]

    def add_node(self, x, y, z, safe=False):
        if safe:
            for i, n in enumerate(self.nodes):
                if x==n.x and y==n.y and z==n.z:
                    return i

        self.nodes.append(Node(x, y, z))
        return len(self.nodes) - 1

    def _import_lines(self):
        entities = gmsh.model.getEntities(1)
        if not len(entities):
            raise Exception('[ERROR] No lines found in geometry file')
        _, line_tags = zip(*entities)
        for tag in line_tags:
            # find start and endpoint nodes
            _, line_nodes_tags = zip(*gmsh.model.getBoundary([(1, tag)]))

            # find group name
            group_tag = gmsh.model.getPhysicalGroupsForEntity(1, tag)
            if group_tag.size == 0:
                group_name = None    # not physical lines
            else:
                group_name = gmsh.model.getPhysicalName(1, group_tag[0])
                if group_name not in self.sections:
                    self.sections.append(group_name)

            # add line
            current_nodes = [self.get_node_tag(n) for n in line_nodes_tags]    # nodes are already imported
            current_line = Line(*current_nodes, group=group_name, tag=tag)
            self.lines.append(current_line)

    def _import_shells(self):
        entities = gmsh.model.getEntities(2)
        if len(entities)==0:
            print('[WARNING] No shell entries in geometry file.')
            return False
        _, shell_tags = zip(*entities)
        for tag in shell_tags:
            # find edges
            _, shell_lines_tags = zip(*gmsh.model.getBoundary([(2, tag)]))

            # add shell
            current_lines = [self.get_line_tag(t) for t in shell_lines_tags]
            current_shell = Shell(current_lines, tag=tag)
            self.shells.append(current_shell)

    def _import_volumes(self):
        entities = gmsh.model.getEntities(3)
        if not len(entities):
            print('[WARNING] No volumes entries in geometry file. Ignore the warning only if you run CFAST mode!')
            return False
        _, vols_tags = zip(*entities)
        for tag in vols_tags:
            # find faces
            _, vols_shells_tags = zip(*gmsh.model.getBoundary([(3, tag)]))

            # find group name
            group_tag = gmsh.model.getPhysicalGroupsForEntity(3, tag)
            if group_tag.size == 0:
                raise Exception(f'Volume {tag} has to be in a physical group')
                # group_name = None    # not physical lines
            else:
                group_name = gmsh.model.getPhysicalName(3, group_tag[0])
                if group_name not in self.sections:
                    self.sections.append(group_name)

            # add volumes
            current_shells = [self.get_shell_tag(t) for t in vols_shells_tags]
            current_vol = Volume(current_shells, group=group_name, tag=tag)
            self.volumes.append(current_vol)


    # read geometry
    def _import_entities(self):
        self._import_nodes()
        self._import_lines()
        self._import_shells()
        self._import_volumes()

    # NO IDEA HOW TO PERSIST LAYERS IN STEP TO GEO IMPORT OR ANY OTHER WAY TO DEFINE PHYSICAL GROUPS
    def read_merged_geom(self, file_path: str):
        gmsh.model.add('mcsteel_merges_model')
        gmsh.merge(file_path)
        gmsh.model.occ.synchronize()
        self._import_entities()

    def read_geo_geom(self, geo_path: str):
        gmsh.open(geo_path)
        gmsh.model.occ.synchronize()
        self._import_entities()

    def read_geom(self, file_path: str):
        ext = file_path.split('.')[-1].lower()
        if ext == 'geo':
            self.read_geo_geom(file_path)
        elif ext in ['step', 'brep', 'iges']:
            self.read_merged_geom(file_path)
        else:
            raise Exception(f'[ERROR] Invalid filetype (file extension). GEO or STEP are required.')


    def close_gmsh(self): gmsh.finalize()

    def find_closest_sections(self, fire_coords: list[float], physical_only=True):
        section_coords = {k: [(None, None, None), 0] for k in self.sections}

        # iterate over elements
        for l in self.lines:
            if physical_only and not l.group:
                continue
            projection = gmsh.model.getClosestPoint(1, l.tag, fire_coords)
            if not self.points_between_shells(fire_coords, projection):
                continue
            distance = np.linalg.norm(np.array(fire_coords) - np.array(projection))
            if section_coords[l.group][1] > distance:
                section_coords[l.group] = [projection, distance]


        # return {k:v[0] for k,v in section_coords.items()}
        return section_coords

    # are the two points at the same storey (space bounded by the shells)
    def points_between_shells(self, p1, p2):
        if all([(sh.level - p1[2]) * (sh.level - p2[2]) > 0 for sh in self.shells]):
            return True
        else:
            return False

    def get_above_shell_lvl(self, pt):
        above_level = 1e5
        for sh in self.shells:
            if is_shell_above_pt(sh, pt) and sh.level < above_level:
                above_level = sh.level
        return above_level



class Node:
    def __init__(self, x: float, y: float, z: float, tag: int = None):
        self.x = x
        self.y = y
        self.z = z
        self.tag = tag

    def __repr__(self): return f'{self.x} {self.y} {self.z}'

    def __str__(self): return f'Node ({self.x} {self.y} {self.z})'


class Line:
    def __init__(self, start: Node, end: Node, group: str = None, tag: int = None):
        self.nodes = [start, end]
        self.group = group
        self.tag = tag

    def __repr__(self): return f'{self.nodes}'

    def __str__(self): return f'Line ({self.nodes})'


class Shell:
    def __init__(self, lines: list[Line], tag: int = None):
        self.lines = lines
        self.level = self._get_level()

    def __repr__(self): return f'{self.level}'

    def __str__(self): return f'Shell ({self.level})'

    def _get_level(self):
        level = 1e9     # super-high
        for l in self.lines:
            level = min([level] + [n.z for n in l.nodes])

        return level


class Volume:
    def __init__(self, shells: list[Shell], group: str = None, tag: int = None):
        self.shells = shells
        self.group = group
        self.name, self.fairshare = self._find_properties()
        self.tag = tag
        self.bbox = self._get_bbox()

    def __repr__(self): return f'{self.group} {self.shells}'

    def __str__(self): return f'Volume ({self.group} {self.shells})'

    def _find_properties(self):
        volume_data = self.group.split('-')
        name = volume_data[0]
        prob_factor = volume_data[1] if len(volume_data) == 2 else 1

        return name, prob_factor

    def _get_bbox(self):
        bbox = [round(i, 3) for i in gmsh.model.getBoundingBox(3, self.tag)]
        self.bbox = bbox
        return bbox


def read_geo_geom(geo_path):
    if not geo_path.endswith('.geo'):
        geo_path += '.geo'
    geometry = Geometry()
    geometry.read_geo_geom(geo_path)
    return geometry


def find_closest_section(fire_coords: list[float], structure: Geometry):
    return structure.find_closest_sections(fire_coords)

def is_shell_above_pt(shell, point):
    if isinstance(point, Node):
        point = (point.x, point.y, point.z)

    sh_bbox = gmsh.model.getBoundingBox(2, shell.tag)
    sh_z_at_pt = gmsh.model.getValue(2, shell.tag, point[:-1])
    if sh_bbox[0] <= point.x <= sh_bbox[3] and sh_bbox[4] <= point.y <= sh_bbox[4] and point[2] < sh_z_at_pt:
        return True
    else:
        return False


if __name__ == '__main__':
    geom = Geometry()
    geom.read_geom('/Users/wk/Library/CloudStorage/OneDrive-AkademiaPożarnicza/01_instytut/2023/05_aamks_for_mezzanines/_analiza/test.geo')
    # geom.close_gmsh()
    # geom1 = Geometry()
    # geom1.read_geom('/Users/wk/Library/CloudStorage/OneDrive-AkademiaPożarnicza/01_instytut/2023/05_aamks_for_mezzanines/_analiza/test.step')
    pass